...
import ora from 'ora';
import { VQPSystem, VQPQuerier, VQPVerifier, QueryBuilder } from '../lib/index.js';
...
program
  .name('vqp')
  .description('VQP (Verifiable Query Protocol) CLI Tool')
  .version('1.0.0');
  ...
  .action(async (options) => {
    ...
    try {
      ...
      if (options.ageCheck) {
        ...
        console.error(chalk.red('Please specify a query using --age-check, --citizenship-check, --income-check, --health-check, or --expr'));
        process.exit(1);
      }
      ...
      console.log(chalk.blue('\n📄 Response:'));
      console.log(chalk.gray(`   Result: ${chalk.bold(vqpResponse.result ? chalk.green('✅ TRUE') : chalk.red('❌ FALSE'))}`));
      ...
      if (options.verify) {
        ...
        try {
          ...
          if (verificationResult.overall) {
            ...
            console.log(chalk.gray(`   Cryptographic proof: ${verificationResult.cryptographicProof ? '✅' : '❌'}`));
            console.log(chalk.gray(`   Metadata: ${verificationResult.metadata ? '✅' : '❌'}`));
          }
        } catch (error) {
          verifySpinner.fail(chalk.red('Verification error'));
          console.error(chalk.red('Verification failed:'), error instanceof Error ? error.message : error);
        }
      }
  ...
  .action(async (options) => {
    ...
    try {
      ...
      if (verificationResult.overall) {
        ...
        console.log(chalk.blue('\n🔐 Verification Details:'));
        console.log(chalk[verificationResult.cryptographicProof ? 'green' : 'red'](`   ${verificationResult.cryptographicProof ? '✅' : '❌'} Cryptographic proof: ${verificationResult.cryptographicProof ? 'Valid' : 'Invalid'}`));
        console.log(chalk[verificationResult.metadata ? 'green' : 'red'](`   ${verificationResult.metadata ? '✅' : '❌'} Metadata: ${verificationResult.metadata ? 'Valid' : 'Invalid'}`));
        process.exit(1);
      }
