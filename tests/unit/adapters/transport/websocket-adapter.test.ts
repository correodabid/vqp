...
import {
  ...
  WebSocketMessage,
} from '../../../../lib/adapters/transport/websocket-adapter.js';
...
describe('WebSocket Transport Adapter - Advanced Tests', () => {
  ...
  describe('Query Processing', () => {
    ...
    it('should reject queries with unauthorized access', async () => {
      const query = createUnauthorizedQuery({ '>=': [{ var: 'annual_income' }, 50000] });
    ...
    it('should handle complex boolean logic queries', async () => {
      const query = createTestQuery({
        and: [{ '>=': [{ var: 'age' }, 18] }, { '==': [{ var: 'citizenship' }, 'US'] }],
      });
  ...
  describe('Performance and Stress Testing', () => {
    ...
    it('should handle rapid successive queries', async () => {
      ...
      const queryResponses = allMessages.filter(
        (msg) => msg.type === 'response' && queries.some((q) => q.id === msg.id)
      );

      // Verify all responses
      assert.ok(
        queryResponses.length >= 3,
        `Expected at least 3 responses, got ${queryResponses.length}`
      );
  ...
  describe('Edge Cases and Error Recovery', () => {
    ...
    it('should handle queries with invalid vocabulary', async () => {
      ...
      assert.strictEqual(response.type, 'error');
      assert.ok(
        response.payload.message.includes('Vocabulary not found') ||
    ...
    it('should handle malformed message structure', async () => {
      ...
      assert.strictEqual(response.type, 'error');
      assert.ok(
        response.payload.code === 'INVALID_MESSAGE' || response.payload.code === 'INVALID_QUERY'
      );
